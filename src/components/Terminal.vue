...
<template>
    <div id="terminal">
        <div id="term_box">
            <textarea v-tooltip="tooltip" 
                spellcheck="false" autofocus id="term_input"
                v-model="termText"
    ...
    export default class Terminal extends Vue {
        ...
        private handleCommand(command: string, input: HTMLInputElement): void {
            if (texts[command]) {
                ...
                if (Object.keys(links).some(((key): boolean => {
        ...
        onDelete(e: Event): void {
            ...
            // rest.
            if (currentLine.substr(0, currentLine.length - 1).length >= this.prompt.length) {
